...
PropertiesPanel.prototype._updateActivation = function(current) {
  ...
  function updateTooltip(element, selector, text) {
    var labelNode = domQuery(selector, element);
    ...
    if (!labelNode) {
     ...
     labelNode.setAttribute('title', text);
  ...
  function getLabel() {
    if(current && current.entries && current.entries.name) {
      return getBusinessObject(element).name ?
     getBusinessObject(element).name : current.entries.name.description || '';
    }
  }
  ...
  function getShortLabel() {
    if(current && current.entries && current.entries.name) {
      return getBusinessObject(element).name ?
      shortenName(getBusinessObject(element).name) : shortenName(current.entries.name.description) || '';
    }
  }


function shortenName(value) {
    return value.length >= 25 ? value.substr(0, 25) + '...' : name;
}

  // inject elements id into header
  updateLabel(panelNode, '[data-label-id]', getShortLabel());

  updateTooltip(panelNode, '[data-shorten-name]', getLabel());
...
PropertiesPanel.prototype._createPanel = function(element, tabs) {
  ...
  var panelNode = domify('<div class="bpp-properties"></div>'),
      headerNode = domify('<div class="bpp-properties-header">' +
        ...
        '<h4 class="label section-subheader" data-toggle="tooltip" title="" data-shorten-name data-label-id></h4>' +
        '<div class="search">' +
  ...
  forEach(tabs, function(tab, tabIndex) {
    ...
    forEach(groups, function(group) {
      ...
      groupNode.querySelector('.group-label').addEventListener('click', function(evt) {
